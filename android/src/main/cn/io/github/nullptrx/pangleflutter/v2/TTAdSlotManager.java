...
public class TTAdSlotManager {
  ...
  public static AdSlot getInterstitialAdAdSlot(Float width, Float height, boolean supportDeepLink) {
    ...
    AdSlot build = new AdSlot.Builder()
        ...
        .setImageAdSize(width == null ? 0 : width.intValue(), height == null ? 0
            : height.intValue()) //根据广告平台选择的尺寸（目前该比例规格仅对穿山甲SDK生效，插屏广告支持的广告尺寸：  1:1, 3:2, 2:3）
        .build();
    return build;
  }
  ...
  public static AdSlot getRewardAdSlot(MethodCall call) {
    ...
    builder.setOrientation(
        (isVertical == null || isVertical) ? TTAdConstant.VERTICAL : TTAdConstant.HORIZONTAL);
    ...
    if (expressArgs != null) {
      ...
      builder.setImageAdSize(w.intValue(), h.intValue());
        ...
        if(!TextUtils.isEmpty(customData)){
            try {
                Map map = new Gson().fromJson(customData, Map.class);
                if(map.size() >0){
                    builder.setCustomData(map) ;// 激励视频开启服务端验证时，透传给adn的自定义数据。
                }
            }catch (Exception ex){}
        }
         */
  ...
  public static AdSlot getFeedListAdSlot(MethodCall call) {
    ...
    AdSlot build = builder
        .setAdStyleType(
            AdSlot.TYPE_EXPRESS_AD)//必传，表示请求的模板广告还是原生广告，AdSlot.TYPE_EXPRESS_AD：模板广告 ； AdSlot.TYPE_NATIVE_AD：原生广告
        // 备注
        // 1:如果是信息流自渲染广告，设置广告图片期望的图片宽高 ，不能为0
        // 2:如果是信息流模板广告，宽度设置为希望的宽度，高度设置为0(0为高度选择自适应参数)
        .setAdCount(count == null ? 0 : count) //请求广告数量为1到3条
        .setSupportDeepLink(isSupportDeepLink)
        //.setGdtNativeAdLogoParams(gdtNativeAdLogoParams)
        .build();
    return build;
  }
