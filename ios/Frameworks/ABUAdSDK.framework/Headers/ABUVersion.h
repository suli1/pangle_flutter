...
int ABUVersionCompareByString(NSString *version1, NSString *version2) {
    NSArray *currents = [version1 componentsSeparatedByString:@"."];
    NSArray *others = [version2 componentsSeparatedByString:@"."];
    
    NSInteger count = currents.count;
    if (count > others.count) {
        count = others.count;
    }
    
    for (NSInteger i = 0; i < count; i++) {
        NSInteger current = [[currents objectAtIndex:i] integerValue];
        NSInteger other = [[others objectAtIndex:i] integerValue];
        if (current > other) {
            return 1;
